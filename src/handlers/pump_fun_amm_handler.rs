...
use crate::handlers::pump_fun_amm_idl::{
    BuyArgs, BuyEvent, CollectCoinCreatorFeeEvent, CreatePoolEvent, DepositArgs, DepositEvent,
    SellArgs, SellEvent, WithdrawArgs, WithdrawEvent,
};
use crate::handlers::{Idl, anchor_event_discriminator, constants};
use crate::types::{
    EventPayload, EventType, FeeCollectionRow, FormattedInstruction, LiquidityRow, PoolCreationRow,
    TradeRow, UnifiedTransaction,
};
use crate::utils::{
    calculate_amm_buy_metrics, calculate_amm_sell_metrics, deserialize_lax,
    find_account_pubkey_in_instruction, get_asset_balances, get_mev_protection, get_priority_fee,
    get_tip, get_trading_platform,
};
            ...
            if !formatted_ix.inner_instructions.is_empty() {
                ...
                if let Some((instruction, accounts, instruction_index)) = instruction_info {
                    let remaining_inner_instructions =
                        &formatted_ix.inner_instructions[instruction_index + 1..];
                    let event =
            ...
            let event_type = match action.instruction {
                DecodedInstruction::CreatePool => {
                    if tx.error.is_some() {
                        let row = PoolCreationRow {
                            ...
                            base_address: find_account_pubkey_in_instruction(
                            ...
                            .map(|p| p.to_string())
                            .unwrap_or_default(),
                            quote_address: find_account_pubkey_in_instruction(
                                &self.decoder.instruction_layouts,
                                "create_pool",
                                "quote_mint",
                                action.instruction_accounts,
                ...
                DecodedInstruction::Buy(a) => {
                    if tx.error.is_some() {
                        let trade_row = TradeRow {
                            ...
                            priority_fee,
                            bribe_fee: tip,
                            coin_creator_fee: 0.0,
                            mev_protection,
                            maker: tx.signers.get(0).cloned().unwrap_or_default(),
                            base_balance: 0.0,
                            quote_balance: 0.0,
                            trade_type: constants::TRADE_TYPE_BUY,
                            protocol: constants::PROTOCOL_PUMPFUN_AMM,
                            platform,
                            pool_address: find_account_pubkey_in_instruction(
                                &self.decoder.instruction_layouts,
                                "buy",
                                "pool",
                                action.instruction_accounts,
                        ...
                        if !whitelist.contains(&potential_quote_address.as_str()) && !is_flipped {
                            println!(
                                "[pumpAmm] Skipping buy trade with non-whitelisted pair: {}/{}",
                        ...
                        let base_decimals =
                            tx.token_decimals.get(&base_address).cloned().unwrap_or(0);
                        let quote_decimals =
                            tx.token_decimals.get(&quote_address).cloned().unwrap_or(0);
                        ...
                        let base_normalized =
                            e.base_amount_out as f64 / 10f64.powi(base_decimals as i32);
                        let quote_normalized =
                            e.quote_amount_in as f64 / 10f64.powi(quote_decimals as i32);
                        ...
                        let (pre_trade_quote_reserves, pre_trade_base_reserves) = match (
                            ...
                            _ => {
                                println!(
                                    "[pumpAmm] WARN: Overflow detected calculating pre-trade reserves for tx {}. Skipping trade.",
                                    tx.signature
                                );
                                continue;
                            }
                        ...
                        let coin_creator_fee =
                            (e.coin_creator_fee as f64 / LAMPORTS_PER_SOL as f64);
                ...
                DecodedInstruction::Sell(args) => {
                    if tx.error.is_some() {
                        ...
                        if !whitelist.contains(&potential_quote_address.as_str()) && !is_flipped {
                            println!(
                                "[pumpAmm] Skipping sell trade with non-whitelisted pair: {}/{}",
                                potential_base_address, potential_quote_address
                            );
                            continue;
                        }
                        ...
                        let base_normalized =
                            e.base_amount_in as f64 / 10f64.powi(base_decimals as i32);
                        let quote_normalized =
                            e.quote_amount_out as f64 / 10f64.powi(quote_decimals as i32);
                        ...
                        let (pre_trade_base_reserves, pre_trade_quote_reserves) = match (
                            pre_trade_base_reserves,
                            pre_trade_quote_reserves,
                        ) {
                            (Some(b), Some(q)) => (b, q),
                ...
                DecodedInstruction::CollectCoinCreatorFee => {
                    if tx.error.is_some() {
                        let fee_row = FeeCollectionRow {
                            ...
                            .map(|p| p.to_string())
                            .unwrap_or_default(),
                            token_0_mint_address: find_account_pubkey_in_instruction(
                                &self.decoder.instruction_layouts,
                                "collect_coin_creator_fee",
                                "quote_mint",
                                action.instruction_accounts,
                        ...
                        let vault_address = find_account_pubkey_in_instruction(
                            &self.decoder.instruction_layouts,
                            "collect_coin_creator_fee",
                            "coin_creator_vault_ata",
                            action.instruction_accounts,
                        ...
                        .map(|p| p.to_string())
                        .unwrap_or_default();
                        let claimed_mint = find_account_pubkey_in_instruction(
                            &self.decoder.instruction_layouts,
                            "collect_coin_creator_fee",
                            "quote_mint",
                            action.instruction_accounts,
                        ...
                        .unwrap_or_default();
                        let quote_decimals =
                            tx.token_decimals.get(&claimed_mint).cloned().unwrap_or(6);
                        let claimed_amount =
                            e.coin_creator_fee as f64 / 10f64.powi(quote_decimals as i32);
                        let fee_row = FeeCollectionRow {
                            signature: tx.signature.to_string(),
                            timestamp: tx.block_time,
                            slot: tx.slot,
