...
use std::{collections::HashMap, fs, time::Duration, str::FromStr};

use crate::{
    database::insert_rows,
    types::{
        BurnRow, EventPayload, EventType, LiquidityRow, MintRow, PoolCreationRow, SupplyLockRow, TokenBalanceMap, TradeRow, TransferRow, WalletHoldingRow
    },
};
...
pub async fn simulator_task(
    ...
    println!("🧪 Loaded and sorted {} scenarios. Starting injection loop...", scenarios.len());
    ...
    for scenario in scenarios {
        ...
        if let Err(e) = injection_result {
            eprintln!("[Simulator] Error injecting event '{}': {}", scenario.event_type, e);
        }
...
async fn inject_single_event(
    ...
    let balances_u64: TokenBalanceMap = scenario.balances.into_iter().map(|(mint, wallet_balances)| {
        let converted_balances = wallet_balances.into_iter().map(|(wallet, balance_str)| {
            (wallet, u64::from_str(&balance_str).unwrap_or(0))
        }).collect();
    ...
    let event_payload = match scenario.event_type.as_str() {
        ...
        _ => return Err(anyhow!("Unknown event type in scenario: {}", scenario.event_type)),
    };
    ...
    let _: () = redis_conn.xadd("event_queue", "*", &[("payload", &payload_data)]).await?;
